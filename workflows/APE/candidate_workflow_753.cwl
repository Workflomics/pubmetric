...
label: WorkflowNo_752
doc: A workflow including the tool(s) MAFFT (EBI), PChopper, MS-Fit, OpenMS, OpenSWATH.
...
steps:
  MAFFT (EBI)_01:
    run: add-path-to-the-implementation/MAFFT (EBI).cwl 
    in:
      MAFFT (EBI)_in_1: input_2
    out: [MAFFT (EBI)_out_1]
  PChopper_02:
    run: add-path-to-the-implementation/PChopper.cwl 
    in:
      PChopper_in_1: MAFFT (EBI)_01/MAFFT (EBI)_out_1
    out: [PChopper_out_1]
  MS-Fit_03:
